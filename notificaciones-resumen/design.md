...
## Decisiones
- 2026-06-25 — **Etiquetado en Slack por user ID**, no por `@nombre`. El campo pide el ID (`U01ABC…`) con helper de cómo obtenerlo (perfil › ⋮ › Copiar id. de miembro). Razón: un `@nombre` enviado vía API llega como texto plano y no genera mención real. *(Pendiente confirmar con Agent Factory si usan ID o `@`.)*
- 2026-06-25 — **Sensibilidad ligada a la gráfica** del KPI (últimos 30 días). Modelo correcto (reu 25-jun): la **franja azul = inputs de límite inferior/superior del usuario; es FIJA** (solo cambia con esos inputs, nunca con la sensibilidad). La sensibilidad define **qué se marca como anomalía respecto a esa franja fija**, creciendo **hacia adentro**: **nula** = solo lo que sale de la franja · **media** = eso + los puntos más cercanos al borde (aunque estén dentro) · **alta** = eso + puntos adicionales hacia el centro. Visual: franja azul quieta + **umbral de sensibilidad punteado** que se mete hacia el centro al subir el nivel; puntos más allá del umbral = rojos. El conteo "se habrían generado N señales" se **deriva de los puntos rojos** (no es mock; nula 4 · media 7 · alta 12). Ventana **30 días** (resuelve el desfase 7d-lista / 30d-gráfica). En prod el BE devuelve la serie + outliers **tag-eados por nivel**; la UI filtra. Razón: feedback 25-jun — hacer tangible la consecuencia de cada nivel sin mover los límites del usuario.
- 2026-06-25 — **Templates alineados al payload real del detector (BADS) + 3 Block Kit de Santiago Q.** El cuerpo **combina narrativa IA + métricas del JSON** (corrige la suposición previa de "sin texto de agente": `title`/`summary`/`hypothesis` traen `ai_provenance` y vienen en es/en/pt; lo auditable son las métricas dentro del texto). Estructura: **agrupado por `scope.resource_refs`** → tarjeta por `finding` (categoría `MISSING_FILES`/`VOLUME_VARIATION` → métricas). **Severidad real** `URGENT` (rojo) / `REQUIRES_ATTENTION` (ámbar); **confianza doble** (`finding.confidence` num + `hypothesis.confidence` enum). **Botones = `recommended_actions[]`** (`go_to`/`create_support_ticket`/`send_message`, label es/en/pt, orden por `priority`); marcadores `[1][2]` resueltos vía `references[]` (parametrización de links). `blast_radius`/`affected_dashboards_count` → chip "Afecta N dashboards". **Slack = 3 plantillas vía header `X-Slack-Template`** (`default`/`compact`/`ultra-compact`); se descartan las variantes inventadas (vertical/carousel) — no existen en el backend. Email con **toggle con/sin gráfica** (gráfica solo en incidentes con serie, p. ej. `VOLUME_VARIATION`). Razón: payload real (24-jun) + JSON Block Kit de Santiago Q. (23-jun) + reunión 25-jun. *(Pendiente con Iván: templates Block Kit parametrizados. Riesgo: generación de imagen en Slack.)*
