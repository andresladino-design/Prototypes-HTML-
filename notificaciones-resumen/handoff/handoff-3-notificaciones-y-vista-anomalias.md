...
**Prototipo:** [`handoff-3-notificaciones-y-vista-anomalias.html`](./handoff-3-notificaciones-y-vista-anomalias.html) — HTML acotado a esta épica (derivado de `../index.html` → vista de **Anomalías** → segmented **Configuración**, paquetes de notificación, y **Gestión**, lista de incidentes). Lo atenuado en ese HTML no hace parte de esta entrega. La barra de filtros de Gestión se ve normal pero no tiene función (su lógica se implementa en el handoff 1); el editor sí conserva "Aplicar un filtro guardado", que es de esta épica.
