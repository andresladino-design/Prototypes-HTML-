...
| [`handoff-limites-sensibilidad.md`](./handoff-limites-sensibilidad.md) | **Sensibilidad = severidad por 2 bandas** (slider continuo) + **mini-dashboard de impacto** + límites por bound, **por categoría** (selector manda) + pill por serie. Todo en **una card** (gráfica+impacto+límites+sensibilidad) | `#sens-chart`, `.sens-kpi-wrap`, `.sens-slider`, `.cat-select`, `#cat-limits`, `#series-block`, `.tm-series-sens` |
...
| [`handoff-anomalias-filtros-notificaciones.md`](./handoff-anomalias-filtros-notificaciones.md) | **Vista de Anomalías** (3 épicas, cada cambio de UI enumerado): **Filtros** multiselect (O/Y) + guardados, sin Severidad · **Notificaciones de incidentes = paquetes de reglas** (scope/momento/updates/canales) · **Resumen consolidado** sección propia + flujo completo · empty states/diagramas/motion. **Incluye Guía de implementación** (modelo BADS: incident.status/severity/problem_category + eventos Pulsar; mapeo a componentes desyk de `fe-solutions-mf`; shapes, máquinas de estado, validaciones). Origen: Granola 30-jun + Ohana | `#viewAnomalias`, `#anFMenu`, `#anCfgNotif` (`.an-pkg*`), `#anCfgResumen`, `.an-onb-*` |
...
- **Severidad = sensibilidad** (2 bandas: rojo = fuera de límites/urgente, amarillo = zona de atención/medio). Slider continuo 0–100% + **mini-dashboard de impacto** (3 tiles). Todo **por categoría** (el selector manda límites + slider + grilla), con herencia del global. Gráfica + impacto + límites + sensibilidad en **una sola card**.
