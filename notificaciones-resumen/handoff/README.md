...
| [`handoff-anomalias-filtros-notificaciones.md`](./handoff-anomalias-filtros-notificaciones.md) | **Vista de Anomalías** (cada cambio de UI enumerado). **Actualizado 30-jun pm:** **Filtros** = **Entidades afectadas** (tablero+recurso en un bloque OR) + **Fecha dinámica t-n** + guardados, sin Severidad · **Notificaciones = paquetes de reglas** con alcance (entidades+tipo) y **dos entregas: aviso por evento + resumen consolidado (t-n)** en el mismo editor (el resumen dejó de ser sección propia) · empty states/diagramas/motion. **Incluye Guía de implementación** (modelo BADS; mapeo desyk de `fe-solutions-mf`; shapes, máquinas de estado, validaciones). Origen: Granola 30-jun (am+pm) + Ohana | `#viewAnomalias`, `#anFMenu` (Entidad/Fecha), `#anCfgNotif` (`.an-pkg*`, `.an-deliv*`), `.an-onb-*` |
